    ...
    pi.on("session_before_compact", async (event, ctx) => {
      try {
        ...
        const compaction = await cacheCompactor({
          ...
          sessionSnapshot,
          cacheKeyParams: requestCacheKeyParams,
          cacheMetadataOverride,
          customInstructions: event.customInstructions,
          signal: event.signal,
          thinkingLevel: cacheMetadataOverride ? thinkingLevel : requestThinkingLevel,
          streamOptions: {
            apiKey: auth.apiKey,
            ...
            onPayload: (payload, requestModel) => normalizeProviderPayload(payload, requestModel || model, {
              mode: "chat",
              reasoningLevel: cacheMetadataOverride ? reasoningLevel : requestReasoningLevel,
              reasoningReplay,
            }),
          },
