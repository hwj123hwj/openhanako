...
function buildParameters() {
  return Type.Object({
    ...
    resumeFromRunId: Type.Optional(Type.String({
      description: "前次 workflow 的 runId（taskId），用于断点续跑：已完成且 prompt+opts 未变的 agent 节点瞬间返回缓存结果，首个变化位置起重新执行。",
    })),
  });
}
    ...
    execute: async (_toolCallId, params, _signal, _onUpdate, ctx) => {
      ...
      if (params.resumeFromRunId && jDir) {
        ...
        if (replayJournal.hasEntries) {
          deps.emitEvent?.({ type: "workflow_progress", taskId, message: `从 ${replayJournal.totalEntries} 条缓存恢复` }, parentSessionPath);
        }
      }
